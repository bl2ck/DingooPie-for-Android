...
*/
    ...
    protected static volatile boolean mSystemImeDisabled;
    ...
    private boolean originalSoftInputModeCaptured;
...
*/
    ...
    public static void setSystemImeDisabled(boolean disabled) {
        mSystemImeDisabled = disabled;
        ...
        activity.runOnUiThread(() -> {
            ...
            if (window != null) {
                if (!activity.originalSoftInputModeCaptured) {
                    activity.originalSoftInputMode = window.getAttributes().softInputMode;
                    activity.originalSoftInputModeCaptured = true;
                }
                if (disabled) {
                    ...
                    window.setSoftInputMode(activity.originalSoftInputMode);
                }
            }
            if (disabled) {
                hideSystemImeOnUiThread();
            }
        });
    }

    public static boolean isSystemImeDisabled() {
        return mSystemImeDisabled;
    }

    public static void enforceSystemImePolicy() {
        if (!mSystemImeDisabled || mSingleton == null) {
            return;
        }
        mSingleton.runOnUiThread(SDLActivity::hideSystemImeOnUiThread);
    }

    private static void hideSystemImeOnUiThread() {
        Context context = SDL.getContext();
        if (context == null) {
            return;
        }
        View tokenView = mTextEdit != null ? mTextEdit : mSurface;
        if (mTextEdit != null) {
            mTextEdit.setLayoutParams(new RelativeLayout.LayoutParams(0, 0));
    ...
    public static boolean showTextInput(int x, int y, int w, int h) {
        if (mSystemImeDisabled) {
            enforceSystemImePolicy();
            Log.i(TAG, "System IME request blocked");
            return false;
        }
...
class DummyEdit extends View implements View.OnKeyListener {
    ...
    public InputConnection onCreateInputConnection(EditorInfo outAttrs) {
        if (SDLActivity.isSystemImeDisabled()) {
            return null;
        }
