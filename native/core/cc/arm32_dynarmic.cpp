...
struct DirectRegion
{
    ...
    bool operator==(const DirectRegion& other) const
    {
...
public:
    ...
    {
        std::array<DirectRegion, 5> regions = directRegions(bus);
    ...
    void rebuildPageTable(const Arm32Bus* bus)
    {
        ...
        for (const DirectRegion& region : mappedRegions)
        {
            mapRegion(region);
        }
...
private:
    static std::array<DirectRegion, 5> directRegions(const Arm32Bus* bus)
    {
        return {{
    ...
    void mapRegion(const DirectRegion& region)
    {
        if (!region.data || region.size < 4096u) return;
        uint64_t regionEnd = (uint64_t)region.base + region.size;
    ...
    std::array<DirectRegion, 5> mappedRegions{};
