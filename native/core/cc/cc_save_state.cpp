    ...
    std::string* directory, std::string* relativePath)
{
    ...
    if (!directory || !relativePath || separator == std::string::npos)
    {
        return false;
    }
    *directory = path.substr(0, separator);
    *relativePath = path.substr(separator + 1);
    return true;
}
    ...
    size_t maxSize)
{
    ...
    if (!output || !splitSaveStatePath(path, &directory, &relativePath))
    {
        return false;
    }
...
static bool writeSaveStateFile(const std::string& path,
    const std::vector<uint8_t>& data)
{
    ...
    FILE* file = platformAndroidOpenSaveFile(directory, relativePath, "wb");
    if (!file)
    {
        return false;
    }
