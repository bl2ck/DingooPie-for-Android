...
static void drawAndroidSystemText(const char* text, int x, int y, int pixelSize, SDL_Color color)
{
...
static int androidCompactButtonTextSize(const SDL_Rect& rect)
{
...
static SDL_Rect androidPanelRowRect(const SDL_Rect& panel, int row)
{
    ...
    return SDL_Rect{ panel.x + horizontalInset,
        panel.y + androidUiMetric(kAndroidMenuRowTop) +
            row * androidUiMetric(kAndroidMenuRowHeight + kAndroidMenuRowGap),
...
{
    int contentHeight = std::max(0,
        rowCount * androidUiMetric(kAndroidMenuRowHeight) +
        std::max(0, rowCount - 1) * androidUiMetric(kAndroidMenuRowGap));
...
{
    static const char* prefix = "android-content://";
    return path.compare(0, strlen(prefix), prefix) == 0;
}
