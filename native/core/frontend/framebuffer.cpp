...
int framebufferInitialize(NativeRuntime* runtime)
{
    for (size_t i = 0; i < sizeof(kLcdFramebufferAliases) / sizeof(kLcdFramebufferAliases[0]); ++i)
    {
        ...
        RuntimeError err = nativeRuntimeMapMemory(runtime, kLcdFramebufferAliases[i],
            sizeof(s_framebufferPixels), RUNTIME_PROT_ALL, s_framebufferPixels);
        if (err)
        {
            printf("framebuffer: failed to map s_framebufferPixels alias 0x%08x: %u (%s)\n",
