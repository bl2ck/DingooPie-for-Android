...
static uint64_t fileModifiedTime(const std::string& path)
{
    ...
    return platformGetStorageFileModifiedTime(
        parts.directory, parts.relativePath) / 1000ull;
}
    ...
    SaveStateFormat format, int slot)
{
    ...
    if (!info.path.empty())
    {
        info.exists = saveFileExists(info.path);
        if (info.exists)
        {
    ...
    SaveStateProgressCallback progressCallback, void* progressUserData)
{
    ...
    if (!saveFileReplace(path, bytes.data(), bytes.size()))
    {
    ...
    SaveStateFormat format, int slot)
{
    ...
    if (!saveFilePathSplit(statePath, &stateParts) ||
        !saveFilePathSplit(thumbnailPath, &thumbnailParts) ||
        stateParts.directory != thumbnailParts.directory)
    {
        return false;
    }
    bool stateDeleted = platformDeleteStorageFile(
        stateParts.directory, stateParts.relativePath);
    bool thumbnailDeleted = platformDeleteStorageFile(
        thumbnailParts.directory, thumbnailParts.relativePath);
