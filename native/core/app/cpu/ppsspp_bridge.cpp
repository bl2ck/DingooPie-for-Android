...
static std::vector<DirectMemoryRegion> g_fastRegions;
...
static void rebuildFastMemoryRegions(NativeRuntime* runtime)
{
    ...
    for (size_t i = 0; i < regionCount; ++i)
    {
        ...
        DirectMemoryRegion fastRegion;
        fastRegion.data = region.data;
        fastRegion.base = region.start;
        fastRegion.size = region.size;
...
static uint8_t* findFastMemoryPointerExact(uint32_t address, size_t size)
{
    ...
    {
        const DirectMemoryRegion& region = g_fastRegions[g_lastFastRegion];
        if (begin >= region.base && end <= region.end())
        {
            return region.pointer(address);
        }
    }

    for (size_t i = 0; i < g_fastRegions.size(); ++i)
    {
        const DirectMemoryRegion& region = g_fastRegions[i];
        if (begin >= region.base && end <= region.end())
        {
            g_lastFastRegion = i;
            return region.pointer(address);
...
static uint32_t maxSizeInFastRegion(uint32_t address)
{
    ...
    for (size_t c = 0; c < 2; ++c)
    {
        ...
        {
            const DirectMemoryRegion& region = g_fastRegions[i];
            if (candidate >= region.base && candidate < region.end())
            {
                uint32_t available = (uint32_t)(region.end() - candidate);
                if (available > best)
                {
                    best = available;
                }
