...
#include "platform/lifecycle_services.h"
#include "platform/automation_services.h"
...
extern "C" JNIEXPORT void JNICALL
Java_com_dingoopie_android_DingooPieActivity_nativeToggleGameLibraryLayout(
    JNIEnv*, jclass)
{
    ...
    const std::string& fileName, const char* mode)
{
    ...
    jclass activityClass = env->GetObjectClass(activity);
    jmethodID method = activityClass ? env->GetMethodID(activityClass,
        "openSiblingGameFileDescriptor",
        "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)I") : NULL;
    ...
    if (pathText && fileNameText && modeText)
    {
        descriptor = (int)env->CallIntMethod(
            activity, method, pathText, fileNameText, modeText);
    }
    if (env->ExceptionCheck())
    {
        env->ExceptionClear();
    ...
    const uint8_t* expected, size_t size)
{
    ...
    bool ok = size <= sizeof(actual) &&
        fsys_fread(actual, 1, (uint32_t)size, stream) == size &&
        fsys_fread(actual + size, 1, 1, stream) == 0 &&
    ...
    const std::string& fileName, const char* mode)
{
    ...
    int descriptor = openAndroidGameSiblingDescriptor(
        gamePath, fileName, effectiveMode);
    if (descriptor < 0)
    {
        return NULL;
    }
    FILE* file = fdopen(descriptor, effectiveMode);
    if (!file)
    {
        close(descriptor);
    }
