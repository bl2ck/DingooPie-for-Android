...
#include "platform/external_launch_services.h"
...
#include "platform/jni_local_ref.h"
...
Uint8 virtualControlAlpha(int alpha)
{
...
static void drawAndroidSystemTextLeftCenteredEllipsized(
    const char* text, const SDL_Rect& rect, int pixelSize, SDL_Color color)
{
    AndroidSystemTextTexture* entry = androidSystemTextTexture(text, pixelSize, color);
    ...
    AndroidSystemTextTexture* ellipsis = androidSystemTextTexture("...", pixelSize, color);
    if (!ellipsis)
    {
        return;
    }

    SDL_bool clipEnabled = SDL_RenderIsClipEnabled(g_renderer);
    SDL_Rect previousClip = {};
    SDL_RenderGetClipRect(g_renderer, &previousClip);
    SDL_Rect textClip = rect;
    textClip.w = std::max(0, rect.w - ellipsis->width);
    SDL_RenderSetClipRect(g_renderer, &textClip);
    ...
    SDL_RenderCopy(g_renderer, entry->texture, NULL, &textDestination);

    SDL_Rect ellipsisClip = rect;
    SDL_RenderSetClipRect(g_renderer, &ellipsisClip);
    SDL_Rect ellipsisDestination =
    {
        rect.x + rect.w - ellipsis->width,
        rect.y + (rect.h - ellipsis->height) / 2,
        ellipsis->width,
        ellipsis->height
    };
    SDL_RenderCopy(g_renderer, ellipsis->texture, NULL, &ellipsisDestination);
    SDL_RenderSetClipRect(g_renderer, clipEnabled ? &previousClip : NULL);
}
...
static void showAndroidSystemToolsMenu(void)
{
    JNIEnv* env = (JNIEnv*)SDL_AndroidGetJNIEnv();
    JniLocalRef<jobject> activity(env, (jobject)SDL_AndroidGetActivity());
    if (!env || !activity)
    ...
    jclass activityClass = env->GetObjectClass(activity);
    jmethodID method = activityClass ? env->GetMethodID(activityClass,
        "showGameLibrarySystemToolsMenu", "(ZZ)V") : NULL;
    if (method)
    {
        env->CallVoidMethod(activity, method,
            androidChineseUi() ? JNI_TRUE : JNI_FALSE,
            g_frontendSettings &&
                g_frontendSettings->gameLibraryLayout == GAME_LIBRARY_LAYOUT_MULTI_COLUMN ?
                JNI_TRUE : JNI_FALSE);
    }
    if (env->ExceptionCheck())
    {
...
static const int kAndroidLibraryHeaderBottomGap = 18;
static const int kAndroidLibraryBrandTextSize = 32;
static const int kAndroidLibraryRowHeight = 52;
static const int kAndroidLibraryFormatIconSize = 34;
...
static int androidLibraryColumnCount(void)
{
    return g_frontendSettings &&
        g_frontendSettings->gameLibraryLayout == GAME_LIBRARY_LAYOUT_MULTI_COLUMN ? 2 : 1;
}
...
static AndroidLibraryLayout androidLibraryLayout(int width, int height)
{
    ...
    layout.columnCount = androidLibraryColumnCount();
    layout.columnGap = 10 * scale;
    int contentWidth = std::max(1, width - 2 * layout.margin);
    if (contentWidth > 920 * scale)
    {
        contentWidth = 920 * scale;
    }
    layout.cardWidth = std::max(1,
        (contentWidth - (layout.columnCount - 1) * layout.columnGap) /
        layout.columnCount);
    ...
    layout.viewportHeight = std::max(1, height - layout.top - 10 * scale);
    ...
    bool compact = layout.cardWidth < 560 * scale;
    int preferredActionWidth = layout.cardWidth < 400 * scale ? 58 * scale :
        (compact ? 76 * scale : 96 * scale);
    ...
    layout.formatTextSize = 15 * scale;
    layout.gameNameTextSize = 20 * scale;
    layout.removeTextSize = 15 * scale;
    return layout;
}

static int androidLibraryGridRowCount(int itemCount, int columnCount)
{
    return itemCount > 0 ? (itemCount + columnCount - 1) / columnCount : 0;
}

static int androidLibraryVisibleGridRowCount(int width, int height)
{
...
{
    AndroidLibraryLayout layout = androidLibraryLayout(width, height);
    int gridRowCount = androidLibraryGridRowCount(count, layout.columnCount);
    int contentHeight = std::max(0, gridRowCount * layout.rowStep - layout.rowGap);
...
static void ensureAndroidLibrarySelectionVisible(void)
{
    ...
    SDL_GetRendererOutputSize(g_renderer, &width, &height);
    AndroidLibraryLayout layout = androidLibraryLayout(width, height);
    int gridRow = g_androidLibrarySelectedRow / layout.columnCount;
    int rowTop = gridRow * layout.rowStep;
...
{
    AndroidLibraryLayout layout = androidLibraryLayout(width, height);
    int contentWidth = layout.columnCount * layout.cardWidth +
        (layout.columnCount - 1) * layout.columnGap;
    int gridRow = row / layout.columnCount;
    int column = row % layout.columnCount;
    return SDL_Rect{ (width - contentWidth) / 2 +
            column * (layout.cardWidth + layout.columnGap),
        layout.top + gridRow * layout.rowStep - g_androidLibraryScrollOffset,
...
static SDL_Rect androidLibrarySystemToolsButtonRect(int width);
...
static void drawAndroidSystemToolsIcon(const SDL_Rect& button,
    const AndroidThemeColors& colors)
{
    const int scale = androidUiScale();
    const int inset = std::max(10 * scale, button.w / 4);
    const int gap = std::max(3 * scale, button.w / 12);
    const int size = std::max(5 * scale, (button.w - 2 * inset - gap) / 2);
    const int left = button.x + (button.w - 2 * size - gap) / 2;
    const int top = button.y + (button.h - 2 * size - gap) / 2;
    const SDL_Color fill = colors.buttonText;
    const SDL_Color outline = colors.iconBorder;
    const SDL_Rect squares[] = {
        { left, top, size, size },
        { left + size + gap, top, size, size },
        { left, top + size + gap, size, size },
        { left + size + gap, top + size + gap, size, size }
    };
    for (const SDL_Rect& square : squares)
    {
        drawAndroidRect(square, fill);
        drawAndroidOutline(square, outline);
    }
}
...
void frontendToggleGameLibraryLayout(void)
{
    ...
    GameLibraryLayoutMode previousMode = g_frontendSettings->gameLibraryLayout;
    g_frontendSettings->gameLibraryLayout =
        previousMode == GAME_LIBRARY_LAYOUT_MULTI_COLUMN ?
        GAME_LIBRARY_LAYOUT_SINGLE_COLUMN : GAME_LIBRARY_LAYOUT_MULTI_COLUMN;
    if (!emulatorSaveSettings(*g_frontendSettings))
    {
        g_frontendSettings->gameLibraryLayout = previousMode;
        return;
    }
...
static std::string androidGameFormatLabel(const std::string& path)
{
    ...
    {
        label[index] = (char)toupper((unsigned char)label[index]);
    }
    return label;
}
...
static void removeAndroidLibraryGameAtRow(int row)
{
    if (isAndroidGameLibraryScanning() || row < 0 ||
        row >= (int)g_androidGamePaths.size())
    {
        return;
    }

    g_androidLibrarySelectedRow = row;
    g_androidLibrarySelectionHighlightVisible = true;
    const std::string path = g_androidGamePaths[(size_t)row];
    if (confirmAndroidGameRemoval(path) && removeAndroidGamePath(path))
    {
...
static bool drawAndroidLibraryScreen(void)
{
    ...
    SDL_Rect systemToolsButton = androidLibrarySystemToolsButtonRect(width);
    drawAndroidRect(systemToolsButton, colors.button);
    drawAndroidOutline(systemToolsButton, colors.buttonBorder);
    drawAndroidSystemToolsIcon(systemToolsButton, colors);
    ...
    {
        AndroidLibraryLayout layout = androidLibraryLayout(width, height);
        int visibleGridRowCount = androidLibraryVisibleGridRowCount(width, height);
        int totalCount = (int)g_androidGamePaths.size();
        int totalGridRowCount = androidLibraryGridRowCount(totalCount, layout.columnCount);
        int firstGridRow = layout.rowStep > 0 ?
            g_androidLibraryScrollOffset / layout.rowStep : 0;
        if (firstGridRow > 0) --firstGridRow;
        int lastGridRow = std::min(totalGridRowCount,
            firstGridRow + visibleGridRowCount + 2);
        int firstIndex = firstGridRow * layout.columnCount;
        int lastIndex = std::min(totalCount, lastGridRow * layout.columnCount);
        ...
        for (int i = firstIndex; i < lastIndex; ++i)
        {
            ...
            int iconSize = layout.formatIconSize;
            ...
            std::string formatLabel = androidGameFormatLabel(
                g_androidGamePaths[(size_t)i]);
            drawAndroidSystemTextCentered(formatLabel.c_str(), icon,
                layout.formatTextSize,
                colors.text);
            ...
            drawAndroidSystemTextLeftCenteredEllipsized(name.c_str(), nameRect,
                layout.gameNameTextSize, colors.text);
            ...
            drawAndroidSystemTextCentered(androidChineseUi() ? kZhRemove : "Remove",
                removeRect, layout.removeTextSize,
                colors.buttonText);
        }
        SDL_RenderSetClipRect(g_renderer, NULL);
        if (totalGridRowCount > visibleGridRowCount)
        {
            ...
            int thumbHeight = std::max(24 * scale,
                viewportHeight * visibleGridRowCount / totalGridRowCount);
...
static void moveAndroidMenuSelection(int direction)
{
    ...
    if (g_androidMenuScreen == ANDROID_MENU_ABOUT)
    {
        int selectedRow = g_androidMenuSelectedRow - ANDROID_ABOUT_AUTHOR_HOMEPAGE;
        int nextRow = androidDirectionalSelectionRow(selectedRow,
            ANDROID_ABOUT_ROW_COUNT - ANDROID_ABOUT_AUTHOR_HOMEPAGE,
            direction, g_androidMenuSelectionHighlightVisible);
        selectAndroidMenuRow(ANDROID_ABOUT_AUTHOR_HOMEPAGE + nextRow, true);
        return;
    }
...
{
    int itemCount = (int)g_androidGamePaths.size();
    if (itemCount <= 0 || direction == 0)
    {
        return;
    }
    if (!g_androidLibrarySelectionHighlightVisible)
    {
        g_androidLibrarySelectedRow = 0;
        g_androidLibrarySelectionHighlightVisible = true;
        ensureAndroidLibrarySelectionVisible();
    ...
    int columnCount = androidLibraryColumnCount();
    int nextRow = g_androidLibrarySelectedRow;
    if (columnCount == 1)
    {
        nextRow = androidDirectionalSelectionRow(g_androidLibrarySelectedRow,
            itemCount, direction < 0 ? -1 : 1, true);
    }
    else if (direction == -1 && g_androidLibrarySelectedRow % columnCount > 0)
    {
        --nextRow;
    }
    else if (direction == 1 && g_androidLibrarySelectedRow % columnCount + 1 < columnCount &&
        g_androidLibrarySelectedRow + 1 < itemCount)
    {
        ++nextRow;
    }
    else if (direction == -2 && g_androidLibrarySelectedRow >= columnCount)
    {
        nextRow -= columnCount;
    }
    else if (direction == 2 && g_androidLibrarySelectedRow + columnCount < itemCount)
    {
        nextRow += columnCount;
    }
    if (nextRow < 0)
...
static bool handleAndroidLibraryKeyboardShortcut(SDL_Scancode scancode, bool repeat)
{
    if (g_androidMenuScreen != ANDROID_MENU_LIBRARY || repeat)
    {
        return false;
    }
...
static void activateAndroidMenuSelection(void)
{
    ...
    {
        handleAndroidAboutMenuSelection(row);
    }
    else if (g_androidMenuScreen == ANDROID_MENU_MAIN)
    {
...
static bool handleAndroidMenuNavigationEvent(const SDL_Event& ev)
{
    ...
    if (ev.type == SDL_KEYDOWN)
    {
        ...
        if (handleAndroidLibraryKeyboardShortcut(scancode, ev.key.repeat != 0))
        {
            return true;
        }
...
static bool handleAndroidMenuEvent(const SDL_Event& ev)
{
    ...
    if (!frontendGameRunning() && g_androidMenuScreen == ANDROID_MENU_LIBRARY)
    {
        ...
        if (pointInRect(x, y, androidLibrarySystemToolsButtonRect(width)))
        {
            showAndroidSystemToolsMenu();
            return true;
        }
        ...
        for (int i = firstIndex; i < lastIndex; ++i)
        {
            ...
            {
                removeAndroidLibraryGameAtRow(i);
                return true;
            }
...
static void drawVirtualButton(const VirtualControlButton& button)
{
    ...
    else
    {
        ...
        SDL_SetRenderDrawColor(g_renderer, 255, 255, 255,
            virtualControlAlpha(portraitMenuButton ? 235 : (pressed ? 255 : 210)));
    ...
    const VirtualControlButton* buttons, int count, int unit, int centerX, int centerY)
{
    ...
    for (int i = 4; i < 8; ++i)
    {
        ...
        SDL_SetRenderDrawColor(g_renderer, 255, 255, 255,
            virtualControlAlpha(pressed ? 230 : 112));
...
void frontendApplyInputSettings(const EmulatorSettings& settings)
{
    ...
    printf("frontend: input settings system_ime_disabled=%u virtual_controls=%u virtual_control_scale=%d virtual_control_opacity=%d virtual_dpad_type=%s controller_mapping=%s controller_calibration=%s keyboard_mapping=%s\n",
